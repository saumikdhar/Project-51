...
const ProjectCardPending = props => (
  <tr>
    ...
    <td>
      <Link  to={"/adminReviewProject/" + props.project.id}><Button>Review</Button></Link>
    </td>
  </tr>
)
...
export default class project_dash_pending_Component extends Component {
    ...
    projectList(){
      var element;
        ...
        return this.state.pendingProjects.sort(function (a, b){return b.priorityScore - a.priorityScore})
          .map(currentproject => { return <ProjectCardPending project = {currentproject}  key={currentproject.id}/>;
        })
    }
    ...
    render() {
        return (
          <div className={"container"}>
            ...
            <Link  to={"/adminActiveDash"}><Button>Active Projects</Button></Link>
            <Button type="primary">Pending Projects
              <span className="rounded-circle bg-danger text-white">&ensp;{this.state.pendingProjects.length}&ensp;
              </span>
            </Button>
